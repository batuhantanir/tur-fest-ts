...
const sortOptions = [
  ...
  { name: 'Fiyata Göre Artan', key: "cheapest", href: '#', current: false, icon: FaArrowUp },
  { name: 'Fiyata Göre Azalan', key: "most_expensive", href: '#', current: false, icon: FaArrowDown },
]
...
function Tours() {
  ...
  const limit = Number(searchParams.get('limit')) ? Number(searchParams.get('limit')) : 5
  ...
  useEffect(() => {
    axios.get(`${baseUrl}/tours/list?limit=${limit}&offset=${offset}&${searchQuery.split('&').filter((item) => !item.includes('limit') && !item.includes('offset')).join("&")}`)
      .then(function (response) {
        setIsLoading(true)
        if (response.status === 200) {
          setProducts(response.data.data.tours);
    ...
    axios.get(`${baseUrl}/categories`)
      .then(function (response) {
        if (response.status === 200) {
          ...
          setFilters((prev) => { return [...prev, ...categories] });
        }
      })
      .catch(function (error) {
        alert(error.message);
      ...
      <div>
        ...
        <main className="w-full mx-auto max-w-7xl">
          <div className="flex items-baseline justify-between py-6 pb-6 border-b border-gray-200">
            <h1 className="text-4xl font-bold tracking-tight text-gray-900">Turlar</h1>
                        ...
                        <Menu.Item key={option.name}>
                          {({ active }) => (
                            ...
                            >
                              ...
                              <span>{option.icon && <option.icon className='size-3' />}</span>
                            </button>
                          )}
                        </Menu.Item>
                      ))}
          ...
          <section aria-labelledby="products-heading" className="pt-6 ">
                          ...
                          <div className="space-y-4">
                            {section.sub_categories.map((option, optionIdx) => (
                              <div key={option.value} className="flex items-center">
                                ...
                                >
                                  {option.name}{option.total_tours && `(${option.total_tours})`}
                                </label>
                              </div>
                            ))}
                          </div>
            ...
            {products.length != 0 && (isLoading ? <Pagination className={"py-10"}>
              ...
              <Pagination className={"py-10"}>
                <PaginationContent>
                  <PaginationItem>
                    <PaginationPrevious
                      href={`${offset > 0 ? `/tours?limit=${limit}&offset=${offset - limit}&${searchQuery.split('&').filter((item) => !item.includes('limit') && !item.includes('offset')).join("&")}` : "#"}`}
                      className={`${(offset <= 0) && "pointer-events-none text-black/50 hover:text-black/50"}`}
                    />
                  </PaginationItem>
                  <PaginationItem>
                    {offset - limit >= 0 && !isMobile && <PaginationLink href={`/tours?limit${limit}&offset=${offset - limit}&${searchQuery.split('&').filter((item) => !item.includes('limit') && !item.includes('offset')).join("&")}`}>{(offset / limit)}</PaginationLink>}
                    <PaginationLink href='#'>{(offset / limit) + 1}</PaginationLink>
                    {offset + limit < totalProductsCount && !isMobile && <PaginationLink href={`/tours?limit${limit}&&offset=${offset + limit}&${searchQuery.split('&').filter((item) => !item.includes('limit') && !item.includes('offset')).join("&")}`}>{(offset / limit) + 2}</PaginationLink>}
                  </PaginationItem>
                  {
                    !isMobile && <PaginationItem>
                      {offset + (2 * limit) < totalProductsCount && <PaginationEllipsis />}
                    </PaginationItem>
                  }
                  <PaginationItem>
                    <PaginationNext
                      href={offset + limit < totalProductsCount ? `/tours?limit=${limit}&offset=${offset + limit}&${searchQuery.split('&').filter((item) => !item.includes('limit') && !item.includes('offset')).join("&")}` : '#'}
                      className={`${!(offset + limit < totalProductsCount) && "pointer-events-none text-black/50 hover:text-black/50"}`}
                    />
                  </PaginationItem>
                </PaginationContent>
              </Pagination>
